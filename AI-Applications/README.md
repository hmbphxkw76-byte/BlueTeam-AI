...
### 🧩 AI-300 Frameworks 集成
80+ API 端点 · 覆盖 7 大类别（向量数据库 / Agent 框架 / RAG 框架 / 模型服务 / 供应链 / 对抗性 ML / 多模态）· 靶机训练专用模拟后端

### 🔐 多模式认证引擎
3 通道（Cookie / API Key / JWT）独立启停 · 3 策略（any / cookie_apikey_and / high_security）运行时切换 · API Key 角色模型（admin/user/viewer）· 401 响应泄露认证信息用于靶机侦察训练
...
# —— 多模式认证 — 启停控制 ——
LAB_AUTH_MODE_COOKIE=1             # Cookie Session
LAB_AUTH_MODE_APIKEY=1             # API Key
LAB_AUTH_MODE_JWT=1                # JWT Token
...
```
src/llamafw/
├── app.py              # FastAPI 应用主体（280+ 路由）
├── config.py           # 配置管理（环境变量、路径、模型）
├── core.py             # 核心逻辑（LLM 调用、工具、安全过滤）
├── database.py         # 异步 SQLite（12 张表，WAL 模式）
...
- Fernet 加密 Session
- 多模式认证：Cookie / API Key / JWT + 3 种策略组合
- API Key 角色模型（admin/user/viewer）权限分级
